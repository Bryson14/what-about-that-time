...
export async function createGroup(name: string): Promise<number> {
  ...
  if (!existing) throw new Error(`Group upsert failed unexpectedly for '${name}'`);
  return existing.id;
}
...
export async function createTag(name: string): Promise<number> {
  ...
  if (!existing) throw new Error(`Tag upsert failed unexpectedly for '${name}'`);
  return existing.id;
}
...
export async function createSubject(name: string): Promise<number> {
  ...
  if (!existing) throw new Error(`Subject upsert failed unexpectedly for '${name}'`);
  return existing.id;
}
